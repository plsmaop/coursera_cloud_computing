...
Application::Application(char *infile) {
    ...
    for (i = 0; i < par->EN_GPSZ; i++) {
        ...
        addressOfMemberNode =
            (Address *)en->ENinit(addressOfMemberNode, par->PORTNUM);
 ...
 *
 * DESCRIPTION:	This function performs all the membership protocol
 * functionalities
 */
void Application::mp1Run() {
    ...
    for (i = 0; i <= par->EN_GPSZ - 1; i++) {
        /*
         * Receive messages from the network and queue them in the membership
         * protocol queue
         */
    ...
    for (i = par->EN_GPSZ - 1; i >= 0; i--) {
        ...
        if (par->getcurrtime() == (int)(par->STEP_RATE * i)) {
            ...
            cout << i << "-th introduced node is assigned with the address: "
                 << mp1[i]->getMemberNode()->addr.getAddress() << endl;
            nodeCount += i;
        }
         ...
         */
            ...
            if ((i == 0) && (par->globaltime % 500 == 0)) {
                log->LOG(&mp1[i]->getMemberNode()->addr, "@@time=%d",
                         par->getcurrtime());
            }
#endif
        }
 ...
 *
 * DESCRIPTION: This function performs all the key value store related
 * functionalities including: 1) Ring operations 2) CRUD operations
 */
void Application::mp2Run() {
    ...
    for (i = 0; i <= par->EN_GPSZ - 1; i++) {
        /*
         * 1) Update the ring
         * 2) Receive messages from the network and queue them in the KV store
         * queue
         */
        if (par->getcurrtime() > (int)(par->STEP_RATE * i) &&
            !mp2[i]->getMemberNode()->bFailed) {
            if (mp2[i]->getMemberNode()->inited &&
                mp2[i]->getMemberNode()->inGroup) {
                // Step 1
                mp2[i]->updateRing();
            }
            // Step 2
    ...
    if (par->getcurrtime() >= TEST_TIME) {
         ...
         **************/
        /**
         * TEST 1: Checks if there are RF * NUMBER_OF_INSERTS CREATE SUCCESS
         * message are in the log
         *
         */
         ...
         * 		   Check whether RF * NUMBER_OF_INSERTS/2 DELETE SUCCESS
         * message are in the log TEST 2: Delete a non-existent key. Check for a
         * DELETE FAIL message in the lgo
         *
         */
        else if (par->getcurrtime() == TEST_TIME &&
                 DELETE_TEST == par->CRUDTEST) {
            deleteTest();
        }  // End of delete test
         ...
         *************/
        /**
         * TEST 1: Read a key. Check for correct value being read in quorum of
         * replicas
         *
         * Wait for some time after TEST 1
         *
         * TEST 2: Fail a single replica of a key. Check for correct value of
         * the key being read in quorum of replicas
         *
         * Wait for STABILIZE_TIME after TEST 2 (stabilization protocol should
         * ensure at least 3 replicas for all keys at all times)
         *
         * TEST 3 part 1: Fail two replicas of a key. Read the key and check for
         * READ FAIL message in the log. READ should fail because quorum
         * replicas of the key are not up
         *
         * Wait for another STABILIZE_TIME after TEST 3 part 1 (stabilization
         * protocol should ensure at least 3 replicas for all keys at all times)
         *
         * TEST 3 part 2: Read the same key as TEST 3 part 1. Check for correct
         * value of the key being read in quorum of replicas
         *
         ...
         *
         * TEST 5: Read a non-existent key. Check for a READ FAIL message in the
         * log
         *
         */
        else if (par->getcurrtime() >= TEST_TIME &&
                 READ_TEST == par->CRUDTEST) {
            readTest();
        }  // end of read test
         ...
         ***************/
        /**
         * TEST 1: Update a key. Check for correct new value being updated in
         * quorum of replicas
         *
         * Wait for some time after TEST 1
         *
         * TEST 2: Fail a single replica of a key. Update the key. Check for
         * correct new value of the key being updated in quorum of replicas
         *
         * Wait for STABILIZE_TIME after TEST 2 (stabilization protocol should
         * ensure at least 3 replicas for all keys at all times)
         *
         * TEST 3 part 1: Fail two replicas of a key. Update the key and check
         * for READ FAIL message in the log UPDATE should fail because quorum
         * replicas of the key are not up
         *
         * Wait for another STABILIZE_TIME after TEST 3 part 1 (stabilization
         * protocol should ensure at least 3 replicas for all keys at all times)
         *
         * TEST 3 part 2: Update the same key as TEST 3 part 1. Check for
         * correct new value of the key being update in quorum of replicas
         *
         * Wait for some time after TEST 3 part 2
         *
         ...
         *
         * TEST 5: Update a non-existent key. Check for a UPDATE FAIL message in
         * the log
         *
         */
        else if (par->getcurrtime() >= TEST_TIME &&
                 UPDATE_TEST == par->CRUDTEST) {
            updateTest();
        }  // End of update test
...
#ifdef DEBUGLOG
        log->LOG(&mp1[removed]->getMemberNode()->addr, "Node failed at time=%d",
                 par->getcurrtime());
#endif
...
#ifdef DEBUGLOG
            log->LOG(&mp1[i]->getMemberNode()->addr, "Node failed at time = %d",
                     par->getcurrtime());
#endif
...
void Application::insertTestKVPairs() {
    ...
    cout << endl
         << "Sent " << testKVPairs.size() << " create messages to the ring"
         << endl;
}
...
void Application::deleteTest() {
    ...
    log->LOG(&mp2[number]->getMemberNode()->addr,
             "DELETE OPERATION KEY: %s at time: %d", invalidKey.c_str(),
             par->getcurrtime());
...
void Application::readTest() {
    ...
    /**
     * Test 1: Test if value of a single read operation is read correctly in
    ...
    /**
     * Test 2: FAIL ONE REPLICA. Test if value is read correctly in quorum
     * number of nodes after ONE OF THE REPLICAS IS FAILED
     */
        ...
        if (replicas.size() < (RF - 1)) {
            cout << endl
                 << "Could not find at least quorum replicas for this key. "
                    "Exiting!!! size of replicas vector: "
                 << replicas.size() << endl;
            log->LOG(&mp2[number]->getMemberNode()->addr,
                     "Could not find at least quorum replicas for this key. "
                     "Exiting!!! size of replicas vector: %d",
                     replicas.size());
            exit(1);
        }
        ...
        for (int i = 0; i < par->EN_GPSZ; i++) {
            if (mp2[i]->getMemberNode()->addr.getAddress() ==
                ...
                if (!mp2[i]->getMemberNode()->bFailed) {
                    ...
                    // Since we fail at most two nodes, one of the replicas must
                    // be alive
    ...
    /**
     * Test 3 part 1: Fail two replicas. Test if value is read correctly in
     * quorum number of nodes after TWO OF THE REPLICAS ARE FAILED
     */
    ...
    if (par->getcurrtime() >= (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME)) {
        ...
        if (par->getcurrtime() ==
            ...
            if (replicas.size() > 2) {
                    ...
                    while (i != par->EN_GPSZ) {
                        if (mp2[i]->getMemberNode()->addr.getAddress() ==
                            ...
                            if (!mp2[i]->getMemberNode()->bFailed) {
                                ...
                                // Since we fail at most two nodes, one of the
                                // replicas must be alive
                                if (replicaIdToFail > 0) {
                                    replicaIdToFail--;
                                }
                            }
                ...
                cout << endl
                     << "Not enough replicas to fail two nodes. Number of "
                        "replicas of this key: "
    ...
    /**
     * Test 4: FAIL A NON-REPLICA. Test if value is read correctly in quorum
     * number of nodes after a NON-REPLICA IS FAILED
     */
    if (par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME +
                               STABILIZE_TIME + LAST_FAIL_TIME)) {
        ...
        for (int i = 0; i < par->EN_GPSZ; i++) {
            if (!mp2[i]->getMemberNode()->bFailed) {
                if (mp2[i]->getMemberNode()->addr.getAddress() !=
                        replicas.at(PRIMARY).getAddress()->getAddress() &&
                    mp2[i]->getMemberNode()->addr.getAddress() !=
                        replicas.at(SECONDARY).getAddress()->getAddress() &&
                    mp2[i]->getMemberNode()->addr.getAddress() !=
                        replicas.at(TERTIARY).getAddress()->getAddress()) {
    ...
    if (par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME +
        ...
        log->LOG(&mp2[number]->getMemberNode()->addr,
                 "READ OPERATION KEY: %s at time: %d", invalidKey.c_str(),
                 par->getcurrtime());
        // This read should fail since at least quorum nodes are not alive
...
void Application::updateTest() {
    ...
    if (par->getcurrtime() == TEST_TIME) {
        ...
        log->LOG(&mp2[number]->getMemberNode()->addr,
                 ...
                 it->first.c_str(), newValue.c_str(), par->getcurrtime());
    ...
    /**
     * Test 2: FAIL ONE REPLICA. Test if value is updated correctly in quorum
     * number of nodes after ONE OF THE REPLICAS IS FAILED
     */
    if (par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME)) {
        // Step 2.a Find a node that is alive and assign it as number
    ...
    /**
     * Test 3 part 1: Fail two replicas. Test if value is updated correctly in
     * quorum number of nodes after TWO OF THE REPLICAS ARE FAILED
     */
    if (par->getcurrtime() >= (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME)) {
        vector<int> nodesToFail;
        ...
        if (par->getcurrtime() ==
            ...
            if (replicas.size() > 2) {
                ...
                // If the code reaches here. Test your stabilization protocol
                cout << endl
                     << "Not enough replicas to fail two nodes. Exiting test "
                        "case !! "
                     << endl;
            }
            if (count == 2) {
        ...
        /**
         * TEST 3 part 2: After failing two replicas and waiting for
         * STABILIZE_TIME, issue an update
         */
        // Step 3.d Wait for stabilization protocol to kick in
        if (par->getcurrtime() ==
            (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME + STABILIZE_TIME)) {
